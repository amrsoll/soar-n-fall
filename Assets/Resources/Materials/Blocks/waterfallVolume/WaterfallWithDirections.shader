...
Shader "Unlit/waterfallStyleMulti"
{
    Properties
    {
        ...
        _isMirrored("Is edge position on other side", int) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {   
                ...
                //if these are changed the waterfall is flipped
                float3 flippedObjectOrigin = (objectOrigin.y,objectOrigin.x,objectOrigin.z);
                float3 flippedWorldPos=(worldPos.y,worldPos.x,worldPos.z);
                
                
                
                float3 placement = _isX*(worldPos.x-objectOrigin.x) + (1-_isX)*(flippedObjectOrigin.x-flippedWorldPos.x);
                //this is p if _isMirrored=0 and -p if _isMirrored=1
                placement = (1 - 2*_isMirrored)*placement;
